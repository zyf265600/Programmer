...
| long int           | 8 个字节 [^1] | -9,223,372,036,854,775,808 到 9,223,372,036,854,775,807 |
...
| **数据类型** | **占用空间** | **有效数字范围**                                             |
| ------------ | ------------ | ------------------------------------------------------------ |
| float        | 4 个字节     | 单精度型占4个字节（32位）内存空间，-3.4\*10\^(-38)～3.4\*10\^(38) (7 个有效数字[^2]) |
| double       | 8 个字节     | 双精度型占8 个字节（64位）内存空间，-1.7\*10\^(-308)～1.7\*10\^(308) (15 个有效数字) |
| long double  | 16 个字节    | 长双精度型 16 个字节（128位）内存空间，-1.2\*10\^(-4932)～1.2\*10\^(4932) (可提供18-19位有效数字) |

[^2]:有效数字指的是 “有几个数字”。3.14
...
- **Float类型：**
  ...
  ————————————————
  版权声明：本文为CSDN博主「赵侠客」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
  原文链接：https://blog.csdn.net/whzhaochao/article/details/12885875

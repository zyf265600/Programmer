...
* 建立一个通用类，类中的成员 数据类型可以**不具体制定**，用一个**虚拟的类型**来代表。
...
这些容器分为**==序列式容器==**和**==关联式容器 (改变输入顺序，比如对数据进行排序)==**两种:
...
void MyPrint(int val) //for_each 出的值已经被解引用了
{
	cout << val << endl;
}
...
⚠️**vector 与普通数组区别：不同之处在于数组是静态空间，而 vector 可以动态扩展**
